...
public interface MovieRepository extends JpaRepository<Movie, Integer> {
    ...
    String getSoundtrackByMovieShowId(Integer movieShowID);

    @Query(value = "SELECT m.title FROM movie m NATURAL JOIN movie_show ms WHERE ms.id_show=?1", nativeQuery = true)
    String getTitleByMovieShowId(Integer movieShowID);
}
